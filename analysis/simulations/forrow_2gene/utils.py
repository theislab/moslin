...
def simulate_data(
    ...
    sim_params = sim.SimulationParameters(
        division_time_std=0.01 * timescale,
        flow_type=flow_type,
        x0_speed=x0_speed,
    ...
    initial_cell = sim.Cell(
        np.array([x0_initial, 0, 0]), np.zeros(sim_params.barcode_length)
    )
    ...
    true_trees["early"] = sim_inf.truncate_tree(
        true_trees["late"], sample_times["early"], sim_params
    )

    # Computing the ground-truth coupling
    couplings = {
        "true": sim_inf.get_true_coupling(true_trees["early"], true_trees["late"])
    }
    ...
    sim_inf.add_conditional_means_and_variances(
        true_trees["late, annotated"], observed_nodes
    )

    ancestor_info = {
        "true tree": sim_inf.get_ancestor_data(
            true_trees["late, annotated"], sample_times["early"]
        )
    }
    ...
    for tree_kind in ["early", "late"]:
        hamming_distances_with_roots[tree_kind] = sim_inf.barcode_distances(
            np.concatenate(
                [barcode_arrays[tree_kind], np.zeros([1, sim_params.barcode_length])]
            )
        )
        hamming_distances[tree_kind] = sim_inf.barcode_distances(
            barcode_arrays[tree_kind]
        )
        fitted_trees[tree_kind] = sim_inf.neighbor_join(
            hamming_distances_with_roots[tree_kind]
        )
    ...
    observed_nodes = [
        n for n in sim_inf.get_leaves(fitted_trees["late"], include_root=False)
    ]
    sim_inf.add_conditional_means_and_variances(fitted_trees["late"], observed_nodes)
    ancestor_info["fitted tree"] = sim_inf.get_ancestor_data(
        fitted_trees["late"], sample_times["early"]
    )
...
def fit_lineageOT(
    ...
    coupling_costs = {}
    coupling_costs["lineageOT, true tree"] = ot.utils.dist(
        rna_arrays["early"], ancestor_info["true tree"][0]
    ) @ np.diag(ancestor_info["true tree"][1] ** (-1))
    ...
    couplings_lot["lineageOT, fitted"] = ot.emd(
        [], [], coupling_costs["lineageOT, fitted tree"]
    )
    for e in epsilons:
        if e >= 0.1:
            f = ot.sinkhorn
        ...
        couplings_true["true " + str(e)] = f(
            [],
            [],
            coupling_costs["lineageOT, true tree"],
            e * np.mean(ancestor_info["true tree"][1] ** (-1)),
        )
        couplings_fitted["fitted " + str(e)] = f(
            [],
            [],
            coupling_costs["lineageOT, fitted tree"],
...
def fit_moslin_gw(
    ...
    for e in epsilons:
        geom_xx = geometry.Geometry(
            cost_matrix=adata[adata.obs["time"].isin([0])].obsp[
                f"cost_matrices_{cost_matrices_key}"
            ],
            scale_cost="max_cost",
            epsilon=e,
        )
        geom_yy = geometry.Geometry(
            cost_matrix=adata[adata.obs["time"].isin([1])].obsp[
                f"cost_matrices_{cost_matrices_key}"
            ],
            scale_cost="max_cost",
            epsilon=e,
...
def anndata_from_sim(
    ...
    X = np.concatenate(
        (sim_dict["rna_arrays"]["early"], sim_dict["rna_arrays"]["late"])
    )
    ...
    cost_matrices_true[
        : sim_dict["true_distances"]["early"].shape[0],
        : sim_dict["true_distances"]["early"].shape[0],
    ] = sim_dict["true_distances"]["early"]
    cost_matrices_true[
        sim_dict["true_distances"]["early"].shape[0] :,
        sim_dict["true_distances"]["early"].shape[0] :,
    ] = sim_dict["true_distances"]["late"]

    cost_matrices_hamming[
        : sim_dict["hamming_distances"]["early"].shape[0],
        : sim_dict["hamming_distances"]["early"].shape[0],
    ] = sim_dict["hamming_distances"]["early"]
    cost_matrices_hamming[
        sim_dict["hamming_distances"]["early"].shape[0] :,
        sim_dict["hamming_distances"]["early"].shape[0] :,
    ] = sim_dict["hamming_distances"]["late"]

    cost_matrices_fitted[
        : sim_dict["fitted_distances"]["early"].shape[0],
        : sim_dict["fitted_distances"]["early"].shape[0],
    ] = sim_dict["fitted_distances"]["early"]
    cost_matrices_fitted[
        sim_dict["fitted_distances"]["early"].shape[0] :,
        sim_dict["fitted_distances"]["early"].shape[0] :,
    ] = sim_dict["fitted_distances"]["late"]
...
def independent_coupling(sim_dict):
    sim_dict["couplings"]["independent"] = (
        np.ones(sim_dict["couplings"]["true"].shape)
        / sim_dict["couplings"]["true"].size
    )

    sim_dict["independent_ancestor_error"] = sim_inf.OT_cost(
        sim_dict["couplings"]["independent"], sim_dict["rna_cost"]["early"]
    )
...
def plot_metrics(
    ...
    pal_dict = (
        ...
        else {val: pal[i] for i, val in enumerate(ys_dict)}
    )
...
def plot_trajectory(
    ...
    **kwargs,
    ...
    if subsample:
        ...
        xt = xt[tidx]
        G = G[sidx, :][:, tidx]
    colors = {"early": "#005AB5", "late": "#DC3220", "line": "#808080"}
    size = 4
    ...
    if savefig:
        plt.savefig(
            FIG_DIR / f"{flow_type}_trajectory.png",
            bbox_inches="tight",
            transparent=True,
            dpi=300,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--flow_type", type=str, required=False, default="bifurcation")
